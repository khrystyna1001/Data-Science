...
def get_bill_info(data):
    ...
    try:
        ...
        import re
        disease_match = re.search(r'"disease"\s*:\s*"([^"]+)"', response)
        expense_match = re.search(r'"expense"\s*:\s*"([^"]+)"', response)
        data = {
            'disease': disease_match.group(1) if disease_match else '',
            'expense': expense_match.group(1) if expense_match else ''
        }
...
def msg():
    ...
    if bill_info['expense'] and bill_info['expense'].strip() and bill_info['expense'].isdigit() and int(bill_info['expense']) < int(total_claim_amount) :
    ...
    elif bill_info['expense'] and bill_info['expense'].strip() and bill_info['expense'].isdigit() and int(bill_info['expense']) > int(total_claim_amount) :
